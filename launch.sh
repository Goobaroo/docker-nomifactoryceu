...
if ! [[ -f nomi-ceu-1.7a-server.zip ]]; then
	rm -fr bansoukou config config-overrides groovy libraries local mods scripts *.jar *.zip
	curl -Lo nomi-ceu-1.7a-server.zip 'https://edge.forgecdn.net/files/5682/648/nomi-ceu-1.7a-server.zip' && unzip -u -o nomi-ceu-1.7a-server.zip -d /data
fi
