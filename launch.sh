...
if ! [[ -f nomi-ceu-1.7-alpha-2a-server.zip ]]; then
	rm -fr bansoukou config config-overrides groovy libraries local mods scripts *.jar *.zip
	curl -Lo nomi-ceu-1.7-alpha-2a-server.zip 'https://edge.forgecdn.net/files/4941/627/nomi-ceu-1.7-alpha-2a-server.zip' && unzip -u -o nomi-ceu-1.7-alpha-2a-server.zip -d /data
fi
